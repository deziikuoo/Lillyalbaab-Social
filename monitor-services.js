...
class ServiceMonitor {
  constructor() {
    this.services = {
      ...
      snapchat: {
        ...
        port: 8000,
        restartCommand: 'cd ../Snapchat-Service && venv/Scripts/python.exe -m uvicorn server.main:app --reload --port 8000'
      }
    };
  ...
  async checkServiceHealth(serviceKey) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        this.log(`✅ ${service.name} is healthy - Uptime: ${Math.round(health.uptime / 60)} minutes`);
      ...
      this.failureCounts[serviceKey] = (this.failureCounts[serviceKey] || 0) + 1;
      this.log(`❌ ${service.name} health check failed (${this.failureCounts[serviceKey]}/${this.maxFailures}): ${error.message}`);
      
      if (this.failureCounts[serviceKey] >= this.maxFailures) {
        this.log(`🚨 ${service.name} has failed ${this.maxFailures} times, attempting restart...`);
            ...
            if (parts.length > 4) {
              ...
              if (pid && pid !== '0') {
                exec(`taskkill /PID ${pid} /F`, (killError) => {
                  if (killError) {
                    this.log(`⚠️ Could not kill process ${pid}: ${killError.message}`);
                  } else {
  ...
  async checkAllServices() {
    ...
    this.log(`📊 Health check complete: ${healthyCount}/${Object.keys(this.services).length} services healthy`);
  }

  start() {
    this.log('🚀 Service monitor started');
    this.log(`📋 Monitoring services: ${Object.keys(this.services).join(', ')}`);
