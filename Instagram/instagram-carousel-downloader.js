...
class InstagramCarouselDownloader {
  ...
  extractCarouselItems(graphqlData) {
    try {
      ...
      if (
        graphqlData.data?.xdt_api__v1__media__shortcode__web_info?.items?.[0]
      ) {
        media =
          graphqlData.data.xdt_api__v1__media__shortcode__web_info.items[0];
      }
      ...
      if (!media) {
        ...
        console.log(
          "Available data keys:",
          Object.keys(graphqlData.data || {})
        );

        // Check if the media is null (private/restricted/deleted)
        if (graphqlData.data?.xdt_shortcode_media === null) {
          console.log(
            "⚠️ Post appears to be private, restricted, or deleted (xdt_shortcode_media is null)"
          );
          console.log(
            "📝 This is why GraphQL fails but snapsave may still work"
          );
        }

        console.log(
          "Full response structure:",
          JSON.stringify(graphqlData.data, null, 2).substring(0, 500) + "..."
        );
        return [];
      }
      ...
      if (
        ...
        console.log(
          `Processing carousel with ${media.edge_sidecar_to_children.edges.length} items`
        );
        ...
        // Direct API format
        console.log(
          `Processing carousel with ${media.carousel_media.length} items`
        );
        media.carousel_media.forEach((item, index) => {
          ...
          const carouselItem = {
            ...
            dimensions: {
              width:
                best?.width || item.image_versions2?.candidates?.[0]?.width,
              height:
                best?.height || item.image_versions2?.candidates?.[0]?.height,
            },
          };
      ...
      console.error(
        "Error extracting carousel items from GraphQL:",
        error.message
      );
      return [];
    }
  ...
  async downloadCarousel(url) {
    try {
      ...
      if (graphqlData) {
        ...
        console.log(
          `GraphQL method found ${carouselItems.length} carousel items`
        );
      }
      ...
      // Check if post is pinned from GraphQL data - try multiple possible paths
      let isPinned = false;
      if (graphqlData?.data?.xdt_shortcode_media?.is_pinned) {
        isPinned = graphqlData.data.xdt_shortcode_media.is_pinned;
      } else if (graphqlData?.data?.shortcode_media?.is_pinned) {
        isPinned = graphqlData.data.shortcode_media.is_pinned;
      } else if (graphqlData?.data?.media?.is_pinned) {
        isPinned = graphqlData.data.media.is_pinned;
      }

      // Log the pinned detection for debugging
      console.log(
        `🔍 Pinned detection for ${shortcode}: ${
          isPinned ? "PINNED" : "NOT PINNED"
        }`
      );
      if (graphqlData?.data) {
        console.log(
          `🔍 GraphQL data keys: ${Object.keys(graphqlData.data).join(", ")}`
        );
      }
      ...
      console.log(
        `✅ Successfully formatted ${formattedItems.length} carousel items${
