...
const App: React.FC = () => {
  ...
  // Send to Telegram
  async function sendToTelegram(itemIndex: number, mediaUrl: string, originalSnapchatUrl?: string) {
    ...
    try {
      const response = await fetch('/send-to-telegram', {
        ...
        body: JSON.stringify({
          mediaUrl,
          originalSnapchatUrl,
          caption: `✨ New ${downloadType} from <a href="https://snapchat.com/add/${currentTarget || 'User not found'}">@${currentTarget || 'User not found'}</a>! 📱`
        })
      })
      ...
      if (response.ok && data.success) {
        ...
        setTelegramErrors(prev => ({ ...prev, [itemIndex]: data.detail || 'Failed to send to Telegram' }))
      }
    } catch (err: any) {
      setSendingStatus(prev => ({ ...prev, [itemIndex]: 'error' }))
