...
const App: React.FC = () => {
  ...
  const clearStorage = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to clear storage and cache for @${currentTarget}? This will allow all posts to be processed again.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        alert(
          `✅ Storage cleared! Deleted ${data.processed_deleted} processed posts and ${data.cache_deleted} cache entries for @${data.username}`
        );
      } else {
  ...
  const clearStoriesCache = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to clear stories cache for @${currentTarget}? This will allow all stories to be processed again.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        alert(
          `✅ Stories cache cleared! Deleted ${data.processed_stories_deleted} processed stories and ${data.stories_cache_deleted} stories cache entries for @${data.username}`
        );
      } else {
  ...
  const fetchDownloads = async (url: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/igdl?url=${encodeURIComponent(url)}`
      );
      ...
      if (isFromGraphQL) {
        ...
        console.log(
          `✅ Detected GraphQL result with ${rawItems.length} unique items`
        );
        processedItems.push(
        ...
        console.log(
          `🔄 Detected snapsave result, deduplicating quality variants...`
        );
      ...
      console.log(
        `🔍 URL Analysis: ${finalUrls.length} total URLs, ${finalUniqueUrls.size} unique URLs`
      );
      if (finalUrls.length !== finalUniqueUrls.size) {
        ...
        Object.entries(urlCounts).forEach(([url, count]) => {
          if (count > 1) {
            console.log(
              `  URL appears ${count} times: ${url.substring(0, 100)}...`
            );
          }
        });
      }
      ...
      if (autoSendToTelegram && processedItems.length > 0) {
        console.log(
          "Auto-send disabled - backend handles all Telegram sending to avoid duplicates"
        );
      }
    } catch (err: any) {
              ...
              {currentTarget && (
                <div className="storage-controls">
                  ...
                  <small className="storage-info">
                    Clear posts cache to re-process posts, or stories cache to
                    re-process stories
                  </small>
                </div>
              )}
